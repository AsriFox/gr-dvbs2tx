...
bbheader_source::sptr bbheader_source::make(int nstreams,
                                            ...
                                            char* dst_address)
{
    return gnuradio::get_initial_sptr(new bbheader_source_impl(nstreams,
                                                               framesize1,
                                                               rate1,
                                                               constellation1,
                                                               pilots1,
 ...
 */
bbheader_source_impl::bbheader_source_impl(int nstreams,
                                           dvbs2_framesize_t framesize1,
                                           dvbs2_code_rate_t rate1,
                                           dvbs2_constellation_t constellation1,
                                           dvbs2_pilots_t pilots1,
...
void bbheader_source_impl::connect(const std::string& host, int port)
{
    if (d_connected)
        disconnect();
...
void bbheader_source_impl::disconnect()
{
    if (!d_connected)
        return;
...
void bbheader_source_impl::build_crc8_table(void)
{
    int r, crc;
 ...
 */
int bbheader_source_impl::add_crc8_bits(unsigned char* in, int length)
{
    int crc = 0;
    int b;
    int i = 0;
...
void bbheader_source_impl::add_bbheader(
    unsigned char* out, int count, int padding, bool nibble, int isi)
{
...
int bbheader_source_impl::gold_to_root(int goldcode)
{
    int g;
    int x = 1;
...
int bbheader_source_impl::crc32_calc(unsigned char* buf, int size, int crc)
{
...
void bbheader_source_impl::crc32_init(void)
{
    unsigned int i, j, k;
...
int bbheader_source_impl::checksum(unsigned short* addr, int count, int sum)
{
    while (count > 1) {
        sum += *addr++;
        count -= 2;
...
inline void bbheader_source_impl::ping_reply(int index)
{
    ...
    csum_ptr = (unsigned short*)(packet[index] + sizeof(struct ether_header) +
                                 sizeof(struct ip));
    type_code = *csum_ptr;
...
inline void bbheader_source_impl::ipaddr_spoof(int index)
{
    unsigned short* csum_ptr;
...
inline void bbheader_source_impl::dump_packet(unsigned char* packet)
{
#ifdef DEBUG
    unsigned char pack;
...
int bbheader_source_impl::work(int noutput_items,
    ...
    while (kbch[i] + produced <= (unsigned int)noutput_items) {
        ...
        while (1) {
            if (packet_fragmented[i] == FALSE) {
                ...
                if (packet[i] != NULL) {
                    last_packet_valid[i] = FALSE;
                    if (((hdr.len - sizeof(struct ether_header) + HEADER_SIZE +
                          ETHER_TYPE_LEN + ether_addr_len) <=
                         ((kbch[i] - (offset - first_offset) - padding) / 8)) &&
                        ((hdr.len - sizeof(struct ether_header) + ETHER_TYPE_LEN +
                          ether_addr_len) < 4096)) {
                        ...
                        bits = hdr.len - sizeof(struct ether_header) + ETHER_TYPE_LEN +
                               ether_addr_len; /* GSE_Length */
                        for (int n = 11; n >= 0; n--) {
                            out[offset++] = bits & (1 << n) ? 1 : 0;
                        }
                        ...
                        ether_addr_len =
                            ETHER_ADDR_LEN; /* disable label re-use for now */
                        /* GSE_data_byte */
                        ptr = (unsigned char*)(packet[i] + sizeof(struct ether_header));
                        for (unsigned int j = 0;
                             j < hdr.len - sizeof(struct ether_header);
                             j++) {
                            bits = *ptr++;
                            for (int n = 7; n >= 0; n--) {
                                out[offset++] = bits & (1 << n) ? 1 : 0;
                        ...
                        if (((kbch[i] - (offset - first_offset) - padding) / 8) >=
                            (HEADER_SIZE + FRAG_ID_SIZE + TOTAL_LENGTH_SIZE +
                             ETHER_TYPE_LEN + ether_addr_len)) {
                            if (((kbch[i] - (offset - first_offset) - padding) / 8) ==
                                (HEADER_SIZE + FRAG_ID_SIZE + TOTAL_LENGTH_SIZE +
                                 ETHER_TYPE_LEN + ether_addr_len)) {
                            ...
                            bits =
                                (kbch[i] - ((offset + GSE_LENGTH_SIZE) - first_offset) -
                                 padding) /
                                8; /* GSE_Length */
                            ...
                            bits = hdr.len - sizeof(struct ether_header) +
                                   ETHER_TYPE_LEN + ether_addr_len; /* Total_Length */
                            total_length[0] = (bits >> 8) & 0xff;
                            total_length[1] = bits & 0xff;
                            crc32_partial[i] =
                                crc32_calc(&total_length[0], 2, 0xffffffff);
                            for (int n = 15; n >= 0; n--) {
                                out[offset++] = bits & (1 << n) ? 1 : 0;
                            }
                            ...
                            ptr = (unsigned char*)&eptr->ether_type;
                            crc32_partial[i] =
                                crc32_calc(ptr, ETHER_TYPE_LEN, crc32_partial[i]);
                            for (int j = 0; j < ETHER_TYPE_LEN; j++) {
                                bits = *ptr++;
                                for (int n = 7; n >= 0; n--) {
                                    out[offset++] = bits & (1 << n) ? 1 : 0;
                            ...
                            ptr = eptr->ether_dhost;
                            crc32_partial[i] =
                                crc32_calc(ptr, ether_addr_len, crc32_partial[i]);
                            for (unsigned int j = 0; j < ether_addr_len; j++) {
                                bits = *ptr++;
                                for (int n = 7; n >= 0; n--) {
                                    out[offset++] = bits & (1 << n) ? 1 : 0;
                            ...
                            /* GSE_data_byte */
                            ptr =
                                (unsigned char*)(packet[i] + sizeof(struct ether_header));
                            if (maxsize == TRUE) {
                                length = MAX_GSE_LENGTH - 1 - FRAG_ID_SIZE -
                                         TOTAL_LENGTH_SIZE - ETHER_TYPE_LEN -
                                         ether_addr_len;
                            } else {
                                length =
                                    (kbch[i] - (offset - first_offset) - padding) / 8;
                            ...
                            packet_length[i] =
                                hdr.len - sizeof(struct ether_header) - length;
            ...
            if (packet_fragmented[i] == TRUE) {
                if (((packet_length[i] + HEADER_SIZE + FRAG_ID_SIZE + sizeof(crc32)) <=
                     ((kbch[i] - (offset - first_offset) - padding) / 8)) &&
                    ((packet_length[i] + HEADER_SIZE + FRAG_ID_SIZE + sizeof(crc32)) <
                     4096)) {
                    /* PDU end */
                    gse = TRUE;
                    ...
                    if (packet_length[i] != 0) {
                        bits = FRAG_ID_SIZE + packet_length[i] +
                               sizeof(crc32); /* GSE_Length */
                    } else {
                    ...
                    if (packet_length[i] < (length - (HEADER_SIZE + FRAG_ID_SIZE))) {
                        padding =
                            ((length - (HEADER_SIZE + FRAG_ID_SIZE)) - packet_length[i]) *
                            8;
                    ...
                    bits =
                        (kbch[i] - (offset + GSE_LENGTH_SIZE - first_offset) - padding) /
                        8; /* GSE_Length */
                    if (bits >= MAX_GSE_LENGTH) {
                        bits = MAX_GSE_LENGTH - 1;
                        maxsize = TRUE;
        ...
        if (gse == TRUE) {
            ...
            if (0) {
                ...
                *udp_packet_ptr++ = frame_number++;
                ...
                try {
                    d_socket->send_to(
                        boost::asio::buffer((void*)(udp_packet), (kbch[i] / 8) + 4),
                        d_endpoint);
        ...
        const uint64_t tagmodcod =
            (uint64_t(root_code[i]) << 32) | (uint64_t(pilot_mode[i]) << 24) |
            (uint64_t(signal_constellation[i]) << 16) | (uint64_t(code_rate[i]) << 8) |
            (uint64_t(frame_size[i]) << 1) | uint64_t(dummy);
